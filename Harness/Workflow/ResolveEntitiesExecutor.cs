...
internal sealed class ResolveEntitiesExecutor : Executor<WorkflowState, WorkflowState>
{
    ...
    public override async ValueTask<WorkflowState> HandleAsync(WorkflowState state, IWorkflowContext context, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                WorkflowExecution.AddStep(state.Context, startedAt, "resolve_entities", "ok");
        ...
        catch (HarnessException exception) { WorkflowExecution.AddStep(state.Context, startedAt, "resolve_entities", "error", null, exception.Error); return WorkflowExecution.Error(state, exception); }
        catch (Exception) { return WorkflowExecution.Unexpected(state, "resolve_entities", startedAt); }
    }
}
