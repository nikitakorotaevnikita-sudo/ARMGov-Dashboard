...
`thought` просим держать коротким. Потолок — 5 шагов и 60 секунд; при исчерпании агент отвечает
...
справку по таблице, выполнить свой `SELECT` или дать финальный ответ. Лимит — 5 шагов и 60 секунд
...
**«📈 Аналитика по запросу»** (отдельная страница, пункт в шапке) — свободный вопрос вместо
...
чипами под вопросом, а какие виды подходят конкретному датасету и какой показать по умолчанию —
...
выше). Список категорий на графике режется по топ-20 с подписью, сколько скрыто. История
вопросов держится в пределах сессии вкладки браузера (без сохранения между перезагрузками). Как
и режим «Глубокий анализ» в чате, страница всегда обращается к `/api/ai/sql` и поэтому
недоступна при сетевом показе прототипа удалённым зрителям — см. [CLAUDE.md](CLAUDE.md),
раздел «Известные хрупкости».
...
```
`index.html` и `style.css` читаются при каждом запросе — после правки фронта достаточно
скопировать изменённый файл в выходную папку: `copy index.html bin\Release\net10.0\`
(аналогично для `style.css`).
...
таблицы с учётными данными) и сборку датасета для графика (`/api/ai/dataset/probe`, а также
поле `dataset` в ответе цикла агента — типы колонок, источник, согласованность `rowCount`/
`truncated`, совпадение значений с SQL-шагом). 176 проверок, ожидаемый результат —
`PASS=176 FAIL=0`. Проверки цикла агента требуют живой модели: при её недоступности они падают,
остальные остаются зелёными. Секция обычного ИИ-чата выполняется отдельно и при недоступности
LLM фиксирует graceful-ошибку.
...
серии рядом при нескольких показателях, вырожденные случаи (пустой датасет, отрицательные
значения, логические поля). Чистые функции — ни сервер, ни модель не требуются. 26 проверок,
ожидаемый результат — `PASS=26 FAIL=0`.
