...
sequenceDiagram
  ...
  loop до 5 шагов или 120 секунд
...
`thought` просим держать коротким. Потолок — 5 шагов и 120 секунд; при исчерпании агент отвечает
...
справку по таблице, выполнить свой `SELECT` или дать финальный ответ. Лимит — 5 шагов и 120 секунд
...
согласованность `rowCount`/`truncated`, совпадение значений с SQL-шагом). 182 проверки, ожидаемый
результат — `PASS=182 FAIL=0`. Проверки цикла агента требуют живой модели: при её недоступности
они падают, остальные остаются зелёными. Секция обычного ИИ-чата выполняется отдельно и при
...
функции — ни сервер, ни модель не требуются. 34 проверки, ожидаемый результат —
`PASS=34 FAIL=0`.
