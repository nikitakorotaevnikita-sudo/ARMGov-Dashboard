...
| ИИ — глубокий анализ (SQL, только локальные вызовы) | `POST /api/ai/sql` (агент) · `GET /api/ai/sql/check` (проверка запроса) · `GET /api/ai/sql/run` (проверка + исполнение) |
| ИИ — служебное для агента | `GET /api/ai/tools` (каталог инструментов) · `GET /api/ai/tool` (вызов инструмента) · `GET /api/ai/schema` (справка по таблице) |
...
Эндпоинты `/api/ai/sql`, `/api/ai/sql/check`, `/api/ai/sql/run` исполняют произвольный `SELECT`
и поэтому отвечают только на локальные вызовы — при показе прототипа из локальной сети тумблер
у удалённого зрителя работать не будет, это осознанное ограничение.
...
(валидатор и исполнитель SQL, каталог инструментов, справка по схеме, цикл агента). 157 проверок,
ожидаемый результат — `PASS=157 FAIL=0`. Секция обычного ИИ-чата выполняется отдельно
