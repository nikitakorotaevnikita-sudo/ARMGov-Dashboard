...
| ИИ — служебное для агента | `GET /api/ai/tools` (каталог инструментов) · `GET /api/ai/tool` (вызов инструмента) |
...
таблицы с учётными данными). 161 проверка, ожидаемый результат — `PASS=161 FAIL=0`.
Проверки цикла агента требуют живой модели: при её недоступности они падают, остальные остаются
зелёными. Секция обычного ИИ-чата выполняется отдельно и при недоступности LLM фиксирует
graceful-ошибку.
...
- `index.html` — разметка и логика фронтенда (SPA): обзор, процессы, обращения, ИИ, Бэк-офис, раскладка.
