...
может добавить необязательное поле `chart`: `{from, array, columns, view, keep}` — какой массив,
какие колонки и, для SQL, какие подписи оставить. Сами числа она не называет: сервер
(`DatasetFromJson` / `DatasetFromSqlResult` / сборщик `SqlChartKeep`) берёт их из результата, который реально вернул инструмент,
SQL-шаг или предзагрузка. Модель называет *что* рисовать, но не *чем*; иначе график имел бы
шанс разойтись с текстом ответа и с дашбордом, а заметить это было бы нечем. На этот случай в
наборе smoke-тестов есть регрессия: при источнике `sql` без `keep` строки датасета сверяются
со срезом того же результата, что попал в протокол шагов. Если `keep` задан, сверка с preview
последнего SQL не делается: строки могут прийти из более раннего шага.

**`chart.keep` — на графике те же люди, что в тексте.** Без отбора датасет SQL равен всему
последнему успешному запросу. На стенде это расходилось с ответом двумя способами.

1. Сравнение Босова и Иванова. `ILIKE '%Иванов%'` вернул несколько строк, включая
   «Концева Надежда Ивановна». Текст говорил про двоих, график показывал всю выборку.
2. Тот же вопрос двумя запросами. Первый нашёл только Босова Александра — 972 выполненных.
   Второй — топ по «%иванов%»: Иванов Иван Иванович 51 730 и ещё однофамильцы. Текст сравнил
   51 730 и 972. График взял только второй запрос, Босова на нём не было.

`keep` — массив подписей, скопированных точно как в колонке результата, в порядке столбиков.
Чисел в нём нет. Сборщик ищет каждую подпись по всем успешным SQL прогона с конца: более поздний
запрос перекрывает ранний по тому же имени. Совпадение точное, после `Trim`, без `ILIKE`.
Пустой `keep` — прежнее поведение, весь последний SQL (рейтинг, топ-N). Поле действует только
при `from=sql`. Ненайденные имена попадают в `keepMissing`; если не найдено никого, графика нет.
Сборщик без модели: `dotnet run --project tools/harness-tests -- ChartKeep`, `PASS=8 FAIL=0`.

Прогон 22.09.2026, экран «Аналитика по запросу», вопрос про выполненные задания Иванова Ивана
Ивановича и Босова Александра. Один SQL вернул две строки, в `answer` стоят `from=sql` и `keep`
из этих же имён. Столбики совпали с таблицей шага: Босов Александр 629, Иванов Иван Иванович 195.
Два шага, 26,7 с. Тот же день, чат с «Глубоким анализом»: «сколько поручений создано за последний
квартал» закрылось инструментом `process` с `period=quarter`, без своего SQL — 129 поручений за
июль–сентябрь 2026. Два шага, 12,4 с.

**Вид выбирает код, а не модель.** Подписи категорий на столбиках укорачиваются и ставятся
в шахматном порядке, чтобы длинные ФИО не наезжали друг на друга. Доли: кольцо, если категорий
не больше шести, иначе полосы. `pickViews` (`charts.js`) — чистая функция от формы данных:
...
flowchart TD
  ...
  Start -->|"📈 Аналитика по запросу"| Canvas["Харнес: график по вопросу"]
  Start -->|"💬 Спросить ИИ"| Chat["Чат. Глубокий анализ = тот же харнес"]
...
**Оболочка.** В шапке — логотип Directum, период, профиль и служебные кнопки. Надпись
«АРМ руководителя · Аналитика процессов» из шапки убрана. Шрифт — Segoe UI, как на обложке RX.
Текст чёрный (`#121416`): заголовки, ссылки и активный пункт меню. Заливка кнопок — `#13406d`.
Статусы: в срок `#2e9f0c`, просрочка `#ff3333`, предупреждение `#ff8600`. Раскладка экранов
не менялась.
...
```
`index.html`, `style.css`, `tokens.css` и `charts.js` читаются при каждом запросе — после правки фронта
достаточно скопировать изменённый файл в выходную папку: `copy index.html bin\Release\net10.0\`
(аналогично для `style.css`, `tokens.css` и `charts.js`).
...
согласованность `rowCount`/`truncated`, совпадение значений с SQL-шагом, достижимость плоских колонок разбивки и тренда). Полный прогон 22.09.2026 при живой модели — `PASS=180 FAIL=0`. Проверки цикла агента её и требуют, но при
недоступности не падают: помощник `agent_ok` уводит их в пометку `[ИИ?]`, не давая ни PASS, ни
FAIL, — поэтому на лежащей модели итог будет меньше, и это не ошибка. Туда же уходит
...
- `SqlChartKeep.cs` — отбор строк графика по `chart.keep` из уже выполненных SQL-шагов прогона.
...
- `style.css` — стили компонентов (карточки, кнопки, поля, таблицы, блок «Размышления»).
  `tokens.css` — дневная палитра обложки RX и шрифт Segoe UI. `logo-directum.svg` — логотип.
