...
- Закрыт обход whitelist через совпадение имени физического отношения с именем
  CTE: собственное имя CTE не связывается внутри его тела, а schema-qualified
  отношения всегда проверяются по разрешённому каталогу. Recursive CTE
  по-прежнему отклоняются.
- `Program.SqlRun` и `ReadOnlyExecutor` используют общий процессный
  `HarnessSqlSlots.Instance` с ёмкостью 2; явная инъекция семафора сохранена для
  изолированных тестов.
- RED: новый тест shadowing завершился `PASS=9 FAIL=1`; тест общего gate сначала
  не собирался из-за отсутствующих `HarnessSqlSlots` и двухаргументного
  конструктора.
- GREEN: `sql` — **PASS 11 / FAIL 0**; `results` — **PASS 9 / FAIL 0**;
  `contracts` — **PASS 14 / FAIL 0**; Release build — 0 warning / 0 error.
